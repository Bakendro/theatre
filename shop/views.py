...
def index_page_view(request):
    ...
    return render(request, 'shop/index.html', {'categories': categories, 'form': form, 'user': request.user}) #GET List#


def category_page_view(request, pk): #GET by id
    if request.method == 'GET':
...
def edit_product_page_view(request, pk):
    ...
    if request.method == 'POST': #POST by id
...
def delete_product_page_view(request, pk): #DELETE
    try:
        product = Product.objects.get(id=pk)
        product.delete()
        return redirect(to='products_page')
...
def basket_page_view(request):
    if request.user.is_authenticated:
        if request.method == 'GET': #GET list
...
def add_product_to_basket_view(request, pk): #POST
    if request.user.is_authenticated:
...
def delete_from_basket_view(request, pk): #DELETE
...
def checkout(request):
    # Логика для обработки страницы подтверждения заказа
